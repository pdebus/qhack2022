...
def binary_list(m, n):
    ...
    binary_const = formatstring.format(m)
    reversed_binary_const = binary_const[::-1]
    arr = [int(b) for b in reversed_binary_const]
    # QHACK #
    return arr
...
def basis_states(n):
    ...
    # QHACK #
    arr = [binary_list(m, n) for m in range(2**n)]
    # QHACK #
...
def is_particle_preserving(circuit, n):
    ...
    for state in basis_states(n):
        ...
        in_particles = count_nonzero(state)
        idx = nonzero(output_state)[0]
        for i in idx:
            basis_state = binary_list(i, n)
            out_particles = count_nonzero(basis_state)
            #print(state, in_particles, out_particles, basis_state)
            conserved = conserved and (in_particles == out_particles)
