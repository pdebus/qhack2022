...
def variational_circuit(params, num_vertices):
    ...
    # QHACK #
    qml.templates.StronglyEntanglingLayers(params, wires=range(num_vertices))
...
def train_circuit(num_vertices, H):
    ...
    np.random.seed(42)
    params = np.random.uniform(low=0, high=2 * np.pi, size=(5, num_vertices, 3))
    ...
    for i in range(epochs):
        ...
        if DEBUG and i % 20 == 0:
            print('Iteration = {:},  Energy = {:.8f} Ha'.format(i, energy))

        if conv <= conv_tol:
            break

    return energy / float(num_vertices)
